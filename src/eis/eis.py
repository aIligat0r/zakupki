...
URL = (
    "https://zakupki.gov.ru/epz/order/extendedsearch/orderCsvSettings/download.html?"
...
class EIS:
    ...
    def _formatter(tender: list, product_category: str, search_keyword: str):
        return {
            ...
            "name": tender[3],
            "type": tender[0],
            "link": "https://zakupki.gov.ru/epz/order/notice/ea20/view/common-info.html?regNumber=%s"
            % tender[1].replace("№", ""),
    ...
    def parse(self):
        ...
        for search_rule in self.search_rules:
            for keyword in search_rule["keywords"]:
                logger.info(
                    "search by rule '%s' (keyword: '%s')" % (search_rule, keyword)
                )
                ...
                for tender in df.values:
                    ...
                    if tender not in formatted_results:
                        formatted_results.append(
                            self._formatter(
                                tender, search_rule["product_category"], keyword
                            )
                        )
